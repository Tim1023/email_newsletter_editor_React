...
function* uploadTemplate(action) {
    try {
        const res = yield saveTemplate(action);
        yield put({type: 'TEMPLATE_SAVED', templateId: 111});
        window.showMessage('ok', `Completed ID: 111`, messageTimeout);
    } catch(e) {
        console.error(e);
        window.showMessage('error', e, messageTimeout);
    }
...
function* uploadImage(action) {
	try {
		const res = yield saveImage(action.file);
		console.log(res.data)
		yield put({type: 'IMAGE_ADDED', block: action.block, index: action.index, image: res.data.link});
		window.showMessage('ok', 'Image Updated', messageTimeout);
	} catch(e) {
		console.error(e);
		window.showMessage('error', e, messageTimeout);
	}
